...
from load_intan_rhs.util.read_header import read_header
from load_intan_rhs.util.get_bytes_per_data_block import get_bytes_per_data_block
from load_intan_rhs.util.read_one_data_block import read_one_data_block
from load_intan_rhs.util.notch_filter import notch_filter
from load_intan_rhs.util.data_to_result import data_to_result
...
def read_data(filename, scaled_output=True):
        ...
        if data_present:
            ...
            if header['dc_amplifier_data_saved']:
                data['dc_amplifier_data'] = np.zeros([header['num_amplifier_channels'],
                                                      num_amplifier_samples], dtype=np.uint) * header[
    ...
    if (data_present):
            ...
            if header['dc_amplifier_data_saved']:
                data['dc_amplifier_data'] = np.multiply(19.23,
                                                        (data['dc_amplifier_data'].astype(
                                                            np.int32) - 512))  # units = microvolts

            data['board_adc_data'] = np.multiply(0.003125,
        ...
        if scaled_output and header['notch_filter_frequency'] > 0:
            print_increment = 10
            percent_done = print_increment
